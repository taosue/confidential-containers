...
mkdir -p "${WORK_DIR}/overlay/pause_bundle"
...
# Append the overlay entries into the same cpio archive so the final initrd
# is a single archive. Concatenating two standalone cpio archives does not work
# for the Asterinas initramfs unpack path because only the first archive gets
# processed.
(cd "${WORK_DIR}/overlay" && find . -print0 | sort -z | cpio --null -o -H newc -A -O "${WORK_DIR}/base.cpio" --quiet)

gzip -9 < "${WORK_DIR}/base.cpio" > "${WORK_DIR}/customized-initrd.img"
